        ...
        <div class="row align-end">
            ...
            <div class="flex lg9 md9">
                <va-tabs style="min-width:350px" color="info" grow v-model="tabValue">
                    <template #tabs>
                        <va-tab v-for="tab in tabs" :key="tab.id" :name="tab.id"
        ...
        <div v-if="isDataType" class="row">
            <div style="padding-top:16px;" class="flex lg3 md3">
                ...
                <div style="max-height:66vh;overflow:auto" class="row margin-spacer">
                    <div class="flex">
                        <Filters :filters="filters" :options="reactiveQuery[organism][tabValue]"
                ...
                <div class="row align-center justify-space-between margin-spacer">
                    <div class="flex">
                        <va-chip style="margin:8px" shadow v-for="(k, index) in Object.keys(filters)" :key="index"
                            :icon="reactiveQuery[organism][tabValue].find(opt => opt.key === k).icon" color="info"
                            outline @click="removeFilter(k)">{{ filters[k] }}</va-chip>
                    </div>
                    <div class="flex">
                        <div class="row align-center">
                            <div class="flex">
                                <span style="color:var(--va-info)">{{ `${index + 1}-${experiments.length < index +
                    pageSize ? experiments.length : index + pageSize} of ${experiments.length}
                                        results`}}</span>
                            </div>
                            <div class="flex">
                                <va-button size="large" preset="secondary" color="info" round icon="chevron_left"
                                    :disabled="index - pageSize < 0" @click="index = index - pageSize" />
                            </div>
                            <div class="flex">
                                <va-button size="large" preset="secondary" color="info" round icon="chevron_right"
                                    :disabled="index + pageSize >= experiments.length"
                                    @click="index = index + pageSize" />
                            </div>
                        </div>
                    </div>
                </div>
